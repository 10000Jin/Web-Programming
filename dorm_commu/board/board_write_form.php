...
<script>
  function check_input() {								// 자바 스크립트 함수 완료 버튼 클릭시 호출
      if (!document.board_form.title.value)			// 제목이 없다면
      {
          alert("제목을 입력하세요!");					// 경고창 출력
          document.board_form.title.focus();			// 제목 입력창 커서 이동
          return;
      }
      ...
      document.board_form.submit();						// board_insert.php로 전달
   }
</script>
</head>
<body> 
...
<section>
	...
	<div id="main_content">
		<div id="sub_menu">												<!-- 초록색 서브메뉴 -->
	   	...
	   	<div id="board_box">
		    <div id="board_title">
		    	<span>자유게시판 > 글 쓰기</span>
			</div>
			<div id="board_form">
		    	<form  name="board_form" method="post" action="board_insert.php" enctype="multipart/form-data">		 	<!-- post방식 파일 업로드 위해 enctype을 multipart/form-data로 설정-->
		    	 <ul>
					<li>
		    	...
		    	<ul class="buttons">
					...
					<li><button type="button" onclick="location.href='board_list_form.php'">목록</button></li>
				</ul>
		    	</form>
			</div>
		</div> <!-- board_box -->
