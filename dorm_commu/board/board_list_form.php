...
<link rel="stylesheet" type="text/css" href="/2017136130_텀프로젝트_최만진/css/board.css">
			...
			<ul>
				<li>커뮤니티
					<ul>
						...
						<li><span><a href="/2017136130_텀프로젝트_최만진/dorm_commu/board/board_list_form.php">자유게시판</a></span></li>
...
<?php
	...
	$sql = "select * from board order by num desc";			 // MySQL 명령 설정(레코드 내림차순)
...
?>
					<li>
						...
						<span class="col2"><a href="board_view_form.php?num=<?=$num?>&page=<?=$page?>"><?=$title?></a></span>
...
<?php
	if ($total_page>=2 && $page >= 2){			// 총 페이지가 2이상이고 현재 페이지가 2이상이면
		...
		echo "<li><a href='board_list_form.php?page=$new_page'>◀ 이전</a> </li>";
	}		
	else 
		echo "<li>&nbsp;</li>";					// 아니면 공백
   	...
   	for ($i=1; $i<=$total_page; $i++){			// 게시판 목록 하단에 페이지 링크 번호 출력
		if ($page == $i){     					// 현재 페이지 번호 링크 안함
			...
			echo "<li><a href='board_list_form.php?page=$i'> $i </a><li>";
		}
   	}

   	if ($total_page>=2 && $page != $total_page){	// 총페이지가 2이상이고 현재 페이지가 아니면
		$new_page = $page+1;						// 다음 버튼과 누르면 +1 페이지로 이동
		echo "<li> <a href='board_list_form.php?page=$new_page'>다음 ▶</a> </li>";
	}
	else 
		echo "<li>&nbsp;</li>";						// 아니면 공백
?>
				...
				<ul class="buttons">
					<li><button onclick="location.href='board_list_form.php'">목록</button></li>
					<li>
<?php 
    if($userid) {								// 로그인 시에만 글쓰기 버튼 클릭시 이동가능
?>
