<?php
    ...
    $sql = "select * from board where num = $num";                      // MySQL 명령 설정
	...
	{
		$file_path = "/2017136130_텀프로젝트_최만진/data/board_data/".$copied_name;
		unlink($file_path);                                   // unlink로 첨부 파일 삭제
    }

    $sql = "delete from board where num = $num";               // MySQL 삭제 명령 
    mysqli_query($con, $sql);                                  // MySQL 명령 실행
    mysqli_close($con);                                        // MySQL 서버 연결 종료

    echo "
	     <script>
	         location.href = 'board_list_form.php?page=$page'; 
	     </script>
	   ";                                                      // 목록 보기로 이동
?>
