...
<?php
	...
	$sql = "update board set count=$new_count where num=$num";   // DB서버의 count값 1증가
...
?>		
		    <div>
					...
					<li>
						<?php
							if($file_name) {						// 첨부파일이 있으면
								...
								$file_path = $_SERVER["DOCUMENT_ROOT"]."/2017136130_텀프로젝트_최만진/data/board_data/".$real_name;
								$file_size = filesize($file_path);	// 파일 크기

								echo "▷ 첨부파일 : $file_name ($file_size Byte) &nbsp;&nbsp;&nbsp;&nbsp;
					       		<a href='board_download.php?num=$num&real_name=$real_name&file_name=$file_name&file_type=$file_type'><b>[저장]</b></a><br><br>";
					           	}
						?>
						<?=$content?>							<!-- 글 내용 출력 -->
					</li>		
			    ...
			    <ul class="buttons">								<!-- 버튼들 -->
						<li><button onclick="location.href='board_list_form.php?page=<?=$page?>'">목록</button></li>
<?php
